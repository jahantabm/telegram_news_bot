# -*- coding: utf-8 -*-

import os
import re
import json
import time
import hashlib
from dataclasses import dataclass, field
from datetime import datetime, timezone, timedelta
from difflib import SequenceMatcher
from html import unescape
from urllib.parse import urljoin, urlparse, urlunparse

import requests
import feedparser
from bs4 import BeautifulSoup
from PIL import Image


# =========================================================
# CONFIG
# =========================================================

BOT_TOKEN = os.getenv("BOT_TOKEN", "").strip()
CHAT_ID = os.getenv("CHAT_ID", "").strip()

MAX_POSTS_PER_RUN = 3
MAX_ITEMS_PER_SOURCE = 12

# خبرهای خیلی قدیمی بررسی نشوند
MAX_AGE_HOURS = 12

# حداقل امتیاز برای عبور از فیلتر
MIN_SCORE = 15

# فاصله بین ارسال‌ها
POST_DELAY = 2

# حداکثر تعداد خبر فوری واقعی در هر اجرا
URGENT_SLOTS = 1

# فایل‌های جلوگیری از تکرار
SENT_LINKS_FILE = "sent_links.txt"
SENT_TITLES_FILE = "sent_titles.txt"

REQUEST_TIMEOUT = 20

USER_AGENT = (
    "Mozilla/5.0 (X11; Linux x86_64) "
    "AppleWebKit/537.36 (KHTML, like Gecko) "
    "Chrome/131.0 Safari/537.36 "
    "JAHANTAB-News-Bot/1.0"
)

HEADERS = {
    "User-Agent": USER_AGENT,
    "Accept-Language": "fa-IR,fa;q=0.9,en;q=0.5",
}

SESSION = requests.Session()
SESSION.headers.update(HEADERS)


# =========================================================
# SOURCES
# =========================================================

@dataclass
class Source:
    name: str
    domain: str
    url: str


@dataclass
class NewsItem:
    title: str
    url: str
    source: str
    domain: str

    description: str = ""
    image: str = ""

    published: datetime | None = None

    score: int = 0
    matched: list[str] = field(default_factory=list)

    urgency: int = 0
    is_urgent: bool = False

    topic: str = "تحولات ایران و منطقه"


SOURCES = [
    Source("تابناک", "tabnak.ir", "https://www.tabnak.ir/"),
    Source("فرارو", "fararu.com", "https://fararu.com/"),
    Source("همشهری آنلاین", "hamshahrionline.ir",
           "https://www.hamshahrionline.ir/"),
    Source("آخرین خبر", "akharinkhabar.ir",
           "https://akharinkhabar.ir/"),
    Source("خبر فوری", "khabarfoori.com",
           "https://www.khabarfoori.com/"),
    Source("خبرآنلاین", "khabaronline.ir",
           "https://www.khabaronline.ir/"),
    Source("مهر", "mehrnews.com",
           "https://www.mehrnews.com/"),
    Source("فارس", "farsnews.ir",
           "https://www.farsnews.ir/"),
    Source("ایرنا", "irna.ir",
           "https://www.irna.ir/"),
    Source("ایسنا", "isna.ir",
           "https://www.isna.ir/"),
    Source("صدا و سیما", "iribnews.ir",
           "https://www.iribnews.ir/"),
    Source("باشگاه خبرنگاران جوان", "yjc.ir",
           "https://www.yjc.ir/"),
    Source("تسنیم", "tasnimnews.com",
           "https://www.tasnimnews.com/fa"),
    Source("عصر ایران", "asriran.com",
           "https://www.asriran.com/"),
    Source("مشرق نیوز", "mashreghnews.ir",
           "https://www.mashreghnews.ir/"),
    Source("انتخاب", "entekhab.ir",
           "https://www.entekhab.ir/"),
    Source("الف", "alef.ir",
           "https://www.alef.ir/"),
    Source("ایلنا", "ilna.ir",
           "https://www.ilna.ir/"),

    # NEW
    Source("روز پلاس", "roozplus.com",
           "https://www.roozplus.com/"),
]


TRUSTED_SOURCES = {
    "ایرنا",
    "ایسنا",
    "فارس",
    "تسنیم",
    "مهر",
    "صدا و سیما",
    "ایلنا",
}


# =========================================================
# KEYWORDS
# =========================================================

IRAN_US_TERMS = {
    "ایران": 3,
    "آمریکا": 3,
    "آمریکایی": 3,
    "ترامپ": 3,
    "واشنگتن": 2,
    "پنتاگون": 4,
    "سنتکام": 4,
    "نیروهای آمریکایی": 4,
    "پایگاه آمریکا": 5,
    "پایگاه آمریکایی": 5,
    "نیروهای ایران": 3,
    "سپاه": 3,
    "سپاه پاسداران": 4,
    "ارتش": 2,
    "موشک": 4,
    "موشکی": 4,
    "پدافند": 4,
    "حمله آمریکا": 7,
    "حمله ایران": 7,
    "پاسخ ایران": 6,
    "پاسخ موشکی": 7,
    "درگیری ایران و آمریکا": 8,
    "جنگ ایران و آمریکا": 8,
    "جنگ ایران آمریکا": 8,
    "تحریم": 2,
    "تحریم‌های آمریکا": 3,
    "هسته‌ای": 2,
    "تأسیسات هسته‌ای": 5,
}


HORMUZ_TERMS = {
    "تنگه هرمز": 8,
    "خلیج فارس": 4,
    "دریای عمان": 3,
    "باب‌المندب": 6,
    "باب المندب": 6,
    "دریای سرخ": 5,
    "نفتکش": 4,
    "نفت‌کش": 4,
    "کشتی": 2,
    "کشتیرانی": 3,
    "آب‌های خلیج فارس": 4,
}


RESISTANCE_TERMS = {
    "حزب‌الله": 6,
    "حزب الله": 6,
    "لبنان": 3,
    "حماس": 5,
    "فلسطین": 4,
    "غزه": 5,
    "اسرائیل": 5,
    "اسرائیلی": 5,
    "نتانیاهو": 4,
    "انصارالله": 6,
    "انصار الله": 6,
    "حوثی": 6,
    "حوثی‌ها": 6,
    "یمن": 4,
    "حشدالشعبی": 6,
    "حشد الشعبی": 6,
    "عراق": 3,
    "محور مقاومت": 5,
    "سوریه": 3,
}


MAJOR_EVENT_TERMS = {
    "حمله": 5,
    "حمله موشکی": 8,
    "حمله هوایی": 7,
    "عملیات": 5,
    "عملیات نظامی": 7,
    "انفجار": 6,
    "شلیک": 6,
    "موشک": 5,
    "پاسخ": 4,
    "پاسخ نظامی": 7,
    "آتش‌بس": 6,
    "آتش بس": 6,
    "ترور": 7,
    "کشته": 5,
    "شهید": 5,
    "مجروح": 4,
    "تلفات": 5,
    "اسیر": 4,
    "سقوط": 4,
    "رهگیری": 5,
    "انهدام": 5,
    "اعلام جنگ": 9,
    "آغاز جنگ": 9,
    "پایان جنگ": 8,
    "قطعنامه": 3,
    "اولتیماتوم": 5,
}


URGENT_TERMS = {
    "لحظاتی پیش": 8,
    "دقایقی پیش": 7,
    "همین الان": 8,
    "هم‌اکنون": 8,
    "هم اکنون": 8,
    "در این لحظه": 7,
    "فوری": 3,
    "خبر فوری": 3,
    "تازه‌ترین": 2,
    "تکمیلی": 2,
}


ANALYSIS_TERMS = {
    "تحلیل",
    "یادداشت",
    "کارشناس",
    "گفتگو",
    "گفت‌وگو",
    "بررسی",
    "چرا",
    "چگونه",
    "روایت",
    "تحلیلگر",
    "سناریو",
    "پیش‌بینی",
    "آینده جنگ",
    "پرونده",
}


EXCLUDE_TERMS = {
    "فوتبال",
    "لیگ برتر",
    "استقلال",
    "پرسپولیس",
    "ورزشی",
    "سینما",
    "بازیگر",
    "فیلم",
    "موسیقی",
    "کنسرت",
    "سلامت",
    "پزشکی",
    "بیمارستان",
    "آموزش",
    "دانشگاه",
    "مدرسه",
    "هواشناسی",
    "بورس",
    "قیمت دلار",
    "قیمت طلا",
    "خودرو",
    "مسکن",
    "گردشگری",
    "تکنولوژی",
    "اینترنت",
}


STRONG_TOPIC_TERMS = (
    set(IRAN_US_TERMS)
    | set(HORMUZ_TERMS)
    | set(RESISTANCE_TERMS)
    | set(MAJOR_EVENT_TERMS)
)


# =========================================================
# TEXT HELPERS
# =========================================================

def normalize_text(text: str) -> str:
    if not text:
        return ""

    text = unescape(str(text))

    replacements = {
        "ي": "ی",
        "ى": "ی",
        "ك": "ک",
        "ۀ": "ه",
        "ة": "ه",
        "ؤ": "و",
        "إ": "ا",
        "أ": "ا",
        "ٱ": "ا",
        "\u200c": " ",
        "\u200f": " ",
        "\u200e": " ",
    }

    for a, b in replacements.items():
        text = text.replace(a, b)

    text = re.sub(r"<[^>]+>", " ", text)
    text = re.sub(r"https?://\S+", " ", text)

    text = re.sub(r"\s+", " ", text)

    return text.strip()


def clean_title(title: str) -> str:
    title = normalize_text(title)

    # حذف عبارت‌های تبلیغاتی رایج
    title = re.sub(
        r"^(خبر فوری|فوری|اختصاصی|لحظه به لحظه)\s*[:：\-–—]*\s*",
        "",
        title,
        flags=re.IGNORECASE,
    )

    return title.strip()


def words(text: str) -> set[str]:
    text = normalize_text(text).lower()

    return {
        w for w in re.findall(r"[\w\u0600-\u06FF]+", text)
        if len(w) >= 2
    }


def similarity(a: str, b: str) -> float:
    a = normalize_text(a)
    b = normalize_text(b)

    if not a or not b:
        return 0.0

    return SequenceMatcher(None, a, b).ratio()


def jaccard(a: str, b: str) -> float:
    wa = words(a)
    wb = words(b)

    if not wa or not wb:
        return 0.0

    return len(wa & wb) / len(wa | wb)


def shorten(text: str, limit: int) -> str:
    text = normalize_text(text)

    if len(text) <= limit:
        return text

    text = text[:limit]

    if " " in text:
        text = text.rsplit(" ", 1)[0]

    return text.rstrip("،؛,:- ") + "..."


def escape_html(text: str) -> str:
    text = str(text)

    return (
        text.replace("&", "&amp;")
        .replace("<", "&lt;")
        .replace(">", "&gt;")
        .replace('"', "&quot;")
    )


def persian_digits(value: str) -> str:
    table = str.maketrans(
        "0123456789",
        "۰۱۲۳۴۵۶۷۸۹"
    )

    return str(value).translate(table)


# =========================================================
# URL HELPERS
# =========================================================

def canonical_url(url: str) -> str:
    if not url:
        return ""

    url = url.strip()

    parsed = urlparse(url)

    scheme = parsed.scheme.lower()
    netloc = parsed.netloc.lower()

    if netloc.startswith("www."):
        netloc = netloc[4:]

    path = re.sub(r"/+", "/", parsed.path)

    if path != "/" and path.endswith("/"):
        path = path[:-1]

    # query و fragment حذف می‌شوند
    return urlunparse(
        (scheme, netloc, path, "", "", "")
    )


def same_domain(url: str, domain: str) -> bool:
    try:
        host = urlparse(url).netloc.lower()

        if host.startswith("www."):
            host = host[4:]

        return host == domain or host.endswith("." + domain)

    except Exception:
        return False


# =========================================================
# DATE
# =========================================================

def parse_datetime(value) -> datetime | None:
    if not value:
        return None

    try:
        if hasattr(value, "tm_year"):
            return datetime(
                value.tm_year,
                value.tm_mon,
                value.tm_mday,
                value.tm_hour,
                value.tm_min,
                value.tm_sec,
                tzinfo=timezone.utc,
            )
    except Exception:
        pass

    return None


def age_hours(dt: datetime | None) -> float:
    if not dt:
        return 999999

    if dt.tzinfo is None:
        dt = dt.replace(tzinfo=timezone.utc)

    now = datetime.now(timezone.utc)

    return max(
        0,
        (now - dt).total_seconds() / 3600
    )


def format_time(dt: datetime | None) -> str:
    if not dt:
        return ""

    try:
        local = dt.astimezone(
            timezone(timedelta(hours=3, minutes=30))
        )

        return persian_digits(
            local.strftime("%H:%M")
        )

    except Exception:
        return ""


# =========================================================
# HTTP
# =========================================================

def get_url(url: str):
    try:
        response = SESSION.get(
            url,
            timeout=REQUEST_TIMEOUT,
            allow_redirects=True,
        )

        if response.status_code >= 400:
            return None

        return response

    except Exception as exc:
        print(f"REQUEST ERROR: {url} -> {exc}")

        return None


# =========================================================
# RSS DISCOVERY
# =========================================================

FEED_PATHS = [
    "/rss",
    "/rss.xml",
    "/feed",
    "/feed.xml",
    "/fa/rss",
]


def discover_feeds(source: Source) -> list[str]:
    found = []

    # مسیرهای معمول
    for path in FEED_PATHS:
        found.append(urljoin(source.url, path))

    # autodiscovery از صفحه اصلی
    response = get_url(source.url)

    if response:
        try:
            soup = BeautifulSoup(
                response.text,
                "html.parser"
            )

            for link in soup.find_all(
                "link",
                href=True
            ):
                rel = " ".join(
                    link.get("rel", [])
                ).lower()

                typ = (
                    link.get("type", "")
                    .lower()
                )

                if (
                    "alternate" in rel
                    and (
                        "rss" in typ
                        or "atom" in typ
                        or "xml" in typ
                    )
                ):
                    found.insert(
                        0,
                        urljoin(
                            source.url,
                            link["href"]
                        ),
                    )

        except Exception:
            pass

    result = []

    for url in found:
        url = canonical_url(url)

        if url and url not in result:
            result.append(url)

    return result[:8]


# =========================================================
# RSS PARSER
# =========================================================

def parse_feed(
    feed_url: str,
    source: Source,
) -> list[NewsItem]:

    response = get_url(feed_url)

    if not response:
        return []

    try:
        parsed = feedparser.parse(
            response.content
        )
    except Exception:
        return []

    result = []

    for entry in parsed.entries[:MAX_ITEMS_PER_SOURCE]:

        title = clean_title(
            entry.get("title", "")
        )

        url = canonical_url(
            entry.get("link", "")
        )

        if not title or not url:
            continue

        if not same_domain(
            url,
            source.domain
        ):
            continue

        description = (
            entry.get("summary", "")
            or entry.get("description", "")
            or ""
        )

        description = normalize_text(
            description
        )

        published = (
            parse_datetime(
                entry.get("published_parsed")
            )
            or parse_datetime(
                entry.get("updated_parsed")
            )
        )

        image = ""

        media_content = entry.get(
            "media_content",
            []
        )

        if media_content:
            for media in media_content:
                candidate = (
                    media.get("url")
                    or media.get("href")
                )

                if candidate:
                    image = urljoin(
                        url,
                        candidate
                    )
                    break

        if not image:
            media_thumbnail = entry.get(
                "media_thumbnail",
                []
            )

            if media_thumbnail:
                image = media_thumbnail[0].get(
                    "url",
                    ""
                )

        result.append(
            NewsItem(
                title=title,
                url=url,
                source=source.name,
                domain=source.domain,
                description=description,
                image=image,
                published=published,
            )
        )

    return result


# =========================================================
# ARTICLE ENRICHMENT
# =========================================================

def extract_article_data(
    item: NewsItem
) -> NewsItem:

    response = get_url(item.url)

    if not response:
        return item

    try:
        soup = BeautifulSoup(
            response.text,
            "html.parser"
        )

        # -------------------------------------------------
        # IMAGE
        # -------------------------------------------------

        if not item.image:

            meta = soup.find(
                "meta",
                property="og:image"
            )

            if meta and meta.get("content"):
                item.image = urljoin(
                    item.url,
                    meta["content"]
                )

            if not item.image:
                meta = soup.find(
                    "meta",
                    attrs={
                        "name": "twitter:image"
                    }
                )

                if meta and meta.get("content"):
                    item.image = urljoin(
                        item.url,
                        meta["content"]
                    )

        # -------------------------------------------------
        # DESCRIPTION
        # -------------------------------------------------

        if not item.description:

            meta = soup.find(
                "meta",
                attrs={
                    "name": "description"
                }
            )

            if meta and meta.get("content"):
                item.description = normalize_text(
                    meta["content"]
                )

        # -------------------------------------------------
        # PUBLISHED TIME
        # -------------------------------------------------

        if not item.published:

            meta = soup.find(
                "meta",
                property="article:published_time"
            )

            if meta and meta.get("content"):
                raw = meta["content"]

                try:
                    value = raw.replace(
                        "Z",
                        "+00:00"
                    )

                    dt = datetime.fromisoformat(
                        value
                    )

                    if dt.tzinfo is None:
                        dt = dt.replace(
                            tzinfo=timezone.utc
                        )

                    item.published = dt

                except Exception:
                    pass

    except Exception as exc:
        print(
            f"ARTICLE PARSE ERROR: "
            f"{item.url} -> {exc}"
        )

    return item


# =========================================================
# TOPIC DETECTION
# =========================================================

def detect_topic(
    text: str
) -> str:

    text = normalize_text(text)

    iran_us_score = sum(
        weight
        for term, weight in IRAN_US_TERMS.items()
        if term in text
    )

    hormuz_score = sum(
        weight
        for term, weight in HORMUZ_TERMS.items()
        if term in text
    )

    resistance_score = sum(
        weight
        for term, weight in RESISTANCE_TERMS.items()
        if term in text
    )

    if hormuz_score >= max(
        iran_us_score,
        resistance_score,
        6
    ):
        return "تنگه هرمز و تحولات منطقه"

    if iran_us_score >= max(
        resistance_score,
        8
    ):
        return "ایران و آمریکا"

    if resistance_score >= 7:
        return "تحولات منطقه"

    return "تحولات ایران و منطقه"


# =========================================================
# SCORING
# =========================================================

def calculate_score(
    item: NewsItem
):

    text = normalize_text(
        item.title + " " + item.description
    )

    score = 0
    urgency = 0
    matched = []

    # ایران / آمریکا
    for term, weight in IRAN_US_TERMS.items():
        if term in text:
            score += weight
            matched.append(term)

    # هرمز
    for term, weight in HORMUZ_TERMS.items():
        if term in text:
            score += weight
            matched.append(term)

    # منطقه
    for term, weight in RESISTANCE_TERMS.items():
        if term in text:
            score += weight
            matched.append(term)

    # رویدادهای مهم
    for term, weight in MAJOR_EVENT_TERMS.items():
        if term in text:
            score += weight
            matched.append(term)

    # فوریت
    for term, weight in URGENT_TERMS.items():
        if term in text:
            urgency += weight

    # منبع هرگز نباید فوریت ایجاد کند
    # بنابراین source در text وارد محاسبه نشده است.

    # منبع معتبر کمی امتیاز اضافه می‌گیرد
    if item.source in TRUSTED_SOURCES:
        score += 2

    # تازگی
    age = age_hours(item.published)

    if age <= 1:
        score += 5

    elif age <= 3:
        score += 3

    elif age <= 6:
        score += 1

    # -----------------------------------------------
    # خبر فوری واقعی
    # -----------------------------------------------

    strong_event = any(
        term in text
        for term in (
            "حمله",
            "انفجار",
            "شلیک",
            "حمله موشکی",
            "عملیات نظامی",
            "پاسخ نظامی",
            "ترور",
            "آتش‌بس",
            "اعلام جنگ",
            "آغاز جنگ",
            "کشته",
            "شهید",
            "رهگیری",
            "انهدام",
        )
    )

    very_recent = age <= 3

    item.is_urgent = (
        very_recent
        and strong_event
        and (
            score >= 22
            or urgency >= 8
        )
    )

    item.score = score
    item.urgency = urgency
    item.matched = list(
        dict.fromkeys(matched)
    )

    item.topic = detect_topic(text)

    return item


# =========================================================
# FILTER
# =========================================================

def is_analysis(item: NewsItem) -> bool:

    title = normalize_text(
        item.title
    )

    # اگر واقعاً یک رویداد فوری/مهم باشد،
    # صرف وجود کلمه تحلیل باعث حذف آن نشود.
    if item.is_urgent:
        return False

    return any(
        term in title
        for term in ANALYSIS_TERMS
    )


def is_excluded(item: NewsItem) -> bool:

    text = normalize_text(
        item.title + " " + item.description
    )

    has_strong_topic = any(
        term in text
        for term in STRONG_TOPIC_TERMS
    )

    if has_strong_topic:
        return False

    return any(
        term in text
        for term in EXCLUDE_TERMS
    )


def evaluate(item: NewsItem) -> bool:

    text = normalize_text(
        item.title + " " + item.description
    )

    if not item.title:
        return False

    if is_excluded(item):
        return False

    if is_analysis(item):
        return False

    age = age_hours(item.published)

    # اگر تاریخ داریم، قدیمی‌ها حذف شوند
    if item.published and age > MAX_AGE_HOURS:
        return False

    # خبر خیلی فوری می‌تواند امتیاز پایین‌تری داشته باشد
    if item.is_urgent:
        return (
            item.score >= MIN_SCORE - 5
        )

    return item.score >= MIN_SCORE


# =========================================================
# DEDUP FILES
# =========================================================

def load_lines(filename: str) -> set[str]:

    if not os.path.exists(filename):
        return set()

    try:
        with open(
            filename,
            "r",
            encoding="utf-8"
        ) as f:

            return {
                line.strip()
                for line in f
                if line.strip()
            }

    except Exception:
        return set()


def append_line(
    filename: str,
    value: str
):

    try:
        with open(
            filename,
            "a",
            encoding="utf-8"
        ) as f:

            f.write(
                value.strip() + "\n"
            )

    except Exception as exc:
        print(
            f"FILE WRITE ERROR: "
            f"{filename} -> {exc}"
        )


# =========================================================
# TITLE FINGERPRINT
# =========================================================

def title_fingerprint(title: str) -> str:

    normalized = normalize_text(
        title
    ).lower()

    normalized = re.sub(
        r"[^\w\u0600-\u06FF ]+",
        " ",
        normalized
    )

    normalized = re.sub(
        r"\s+",
        " ",
        normalized
    ).strip()

    return hashlib.sha1(
        normalized.encode("utf-8")
    ).hexdigest()


# =========================================================
# SAME EVENT
# =========================================================

def important_words(text: str) -> set[str]:

    text = normalize_text(text)

    result = set()

    important_groups = [
        IRAN_US_TERMS,
        HORMUZ_TERMS,
        RESISTANCE_TERMS,
        MAJOR_EVENT_TERMS,
    ]

    for group in important_groups:

        for term in group:

            if term in text:
                result.update(
                    words(term)
                )

    return result


def same_event(
    a: NewsItem,
    b: NewsItem
) -> bool:

    # عنوان
    title_sim = similarity(
        a.title,
        b.title
    )

    if title_sim >= 0.78:
        return True

    # کلمات عنوان
    title_jac = jaccard(
        a.title,
        b.title
    )

    if title_jac >= 0.65:
        return True

    # محتوا
    content_a = (
        a.title + " " + a.description
    )

    content_b = (
        b.title + " " + b.description
    )

    content_sim = similarity(
        content_a,
        content_b
    )

    if content_sim >= 0.76:
        return True

    # کلمات مهم
    ia = important_words(
        content_a
    )

    ib = important_words(
        content_b
    )

    if ia and ib:

        overlap = len(
            ia & ib
        ) / len(
            ia | ib
        )

        if overlap >= 0.60:
            return True

    return False


# =========================================================
# CROSS SOURCE DEDUP
# =========================================================

def dedupe_cross_source(
    items: list[NewsItem]
) -> list[NewsItem]:

    # اول خبرهای مهم‌تر
    items = sorted(
        items,
        key=lambda x: (
            x.is_urgent,
            x.score,
            -age_hours(x.published),
            x.source in TRUSTED_SOURCES,
        ),
        reverse=True,
    )

    result = []

    for item in items:

        duplicate = False

        for existing in result:

            if (
                canonical_url(item.url)
                == canonical_url(existing.url)
            ):
                duplicate = True
                break

            if title_fingerprint(
                item.title
            ) == title_fingerprint(
                existing.title
            ):
                duplicate = True
                break

            if same_event(
                item,
                existing
            ):
                duplicate = True
                break

        if not duplicate:
            result.append(item)

    return result


# =========================================================
# IMAGE
# =========================================================

def find_logo_file() -> str:

    candidates = [
        "jahantab_logo_transparent-1.png",
        "logo.png",
        "logo.jpg",
        "logo.jpeg",
    ]

    for filename in candidates:

        if os.path.exists(filename):
            return filename

    return ""


LOGO_FILE = find_logo_file()


def download_image(
    url: str,
    filename: str = "_news_image.jpg"
) -> str:

    if not url:
        return ""

    try:

        response = SESSION.get(
            url,
            timeout=REQUEST_TIMEOUT,
            stream=True
        )

        if response.status_code >= 400:
            return ""

        content_type = (
            response.headers
            .get("Content-Type", "")
            .lower()
        )

        if (
            "image" not in content_type
            and not url.lower().endswith(
                (".jpg", ".jpeg", ".png", ".webp")
            )
        ):
            return ""

        with open(
            filename,
            "wb"
        ) as f:

            for chunk in response.iter_content(
                chunk_size=8192
            ):

                if chunk:
                    f.write(chunk)

        # بررسی معتبر بودن تصویر
        image = Image.open(filename)
        image.verify()

        return filename

    except Exception as exc:

        print(
            f"IMAGE ERROR: {url} -> {exc}"
        )

        try:
            if os.path.exists(filename):
                os.remove(filename)
        except Exception:
            pass

        return ""


def add_logo(
    image_path: str
) -> str:

    if not LOGO_FILE:
        return image_path

    try:

        base = Image.open(
            image_path
        ).convert("RGBA")

        logo = Image.open(
            LOGO_FILE
        ).convert("RGBA")

        # اندازه لوگو
        max_width = max(
            80,
            base.width // 6
        )

        ratio = (
            max_width / logo.width
        )

        new_height = int(
            logo.height * ratio
        )

        logo = logo.resize(
            (
                max_width,
                new_height
            ),
            Image.LANCZOS
        )

        margin = max(
            10,
            base.width // 100
        )

        x = (
            base.width
            - logo.width
            - margin
        )

        y = (
            base.height
            - logo.height
            - margin
        )

        base.alpha_composite(
            logo,
            (x, y)
        )

        output = "_final_news_image.jpg"

        base.convert("RGB").save(
            output,
            "JPEG",
            quality=92
        )

        return output

    except Exception as exc:

        print(
            f"LOGO ERROR: {exc}"
        )

        return image_path


# =========================================================
# CAPTION
# =========================================================

def build_caption(
    item: NewsItem
) -> str:

    title = shorten(
        item.title,
        220
    )

    description = shorten(
        item.description,
        500
    )

    title = escape_html(title)
    description = escape_html(
        description
    )

    source = escape_html(
        item.source
    )

    time_text = format_time(
        item.published
    )

    lines = []

    # عنوان
    lines.append(
        f"🇮🇷 <b>{title}</b>"
    )

    lines.append("")

    # خلاصه
    if description:
        lines.append(
            description
        )

        lines.append("")

    # اطلاعات خبر
    lines.append(
        f"✍🏼 <b>منبع خبر:</b> {source}"
    )

    if time_text:
        lines.append(
            f"🕐 <b>ساعت:</b> {time_text}"
        )

    lines.append("")

    lines.append(
        "━━━━━━━━━━━━━━━━━━"
    )

    # امضای نهایی
    lines.append(
        "🌐 <b>جهان تاب</b> | "
        "<i>آخرین تحولات جهان</i>"
    )

    caption = "\n".join(lines)

    # محدودیت امن کپشن تلگرام
    if len(caption) > 1000:
        caption = caption[:997] + "..."

    return caption


# =========================================================
# TELEGRAM
# =========================================================

def telegram_api(
    method: str
) -> str:

    return (
        f"https://api.telegram.org/bot"
        f"{BOT_TOKEN}/{method}"
    )


def send_photo(
    item: NewsItem,
    image_path: str
) -> bool:

    if not BOT_TOKEN or not CHAT_ID:
        print(
            "ERROR: BOT_TOKEN or CHAT_ID is missing."
        )

        return False

    caption = build_caption(
        item
    )

    keyboard = {
        "inline_keyboard": [
            [
                {
                    "text": "مشاهده خبر",
                    "url": item.url,
                }
            ]
        ]
    }

    data = {
        "chat_id": CHAT_ID,
        "caption": caption,
        "parse_mode": "HTML",
        "reply_markup": json.dumps(
            keyboard,
            ensure_ascii=False
        ),
    }

    try:

        with open(
            image_path,
            "rb"
        ) as photo:

            response = SESSION.post(
                telegram_api("sendPhoto"),
                data=data,
                files={
                    "photo": photo
                },
                timeout=40,
            )

        if response.ok:

            result = response.json()

            if result.get("ok"):
                print(
                    f"SENT: {item.title}"
                )

                return True

        print(
            "TELEGRAM ERROR:",
            response.status_code,
            response.text[:1000]
        )

        return False

    except Exception as exc:

        print(
            f"TELEGRAM SEND ERROR: {exc}"
        )

        return False


# =========================================================
# COLLECT
# =========================================================

def collect_items() -> list[NewsItem]:

    all_items = []

    for source in SOURCES:

        print(
            f"\n========== {source.name} =========="
        )

        feeds = discover_feeds(
            source
        )

        if not feeds:
            print(
                "No feeds discovered."
            )
            continue

        source_items = []

        for feed_url in feeds:

            print(
                f"Checking feed: {feed_url}"
            )

            parsed = parse_feed(
                feed_url,
                source
            )

            source_items.extend(
                parsed
            )

            if len(source_items) >= (
                MAX_ITEMS_PER_SOURCE
            ):
                break

        # حذف تکرار داخلی هر منبع
        seen = set()
        unique_source_items = []

        for item in source_items:

            key = canonical_url(
                item.url
            )

            if key in seen:
                continue

            seen.add(key)

            unique_source_items.append(
                item
            )

        # اول امتیازدهی اولیه
        for item in unique_source_items:

            calculate_score(item)

        # فقط موارد امیدوارکننده را enrich کنیم
        candidates = []

        for item in unique_source_items:

            if (
                item.score >= MIN_SCORE - 5
                or not item.description
                or not item.image
                or not item.published
            ):
                candidates.append(item)

        for item in candidates[:10]:

            extract_article_data(
                item
            )

            calculate_score(
                item
            )

        for item in unique_source_items:

            if evaluate(item):
                all_items.append(item)

        print(
            f"Accepted from {source.name}: "
            f"{sum(1 for x in unique_source_items if evaluate(x))}"
        )

    return all_items


# =========================================================
# SELECT
# =========================================================

def select_items(
    items: list[NewsItem],
    sent_links: set[str],
    sent_titles: set[str],
) -> list[NewsItem]:

    candidates = []

    for item in items:

        link = canonical_url(
            item.url
        )

        title_hash = title_fingerprint(
            item.title
        )

        if link in sent_links:
            print(
                f"SKIP already sent URL: "
                f"{item.title}"
            )
            continue

        if title_hash in sent_titles:
            print(
                f"SKIP already sent title: "
                f"{item.title}"
            )
            continue

        candidates.append(item)

    # جلوگیری از تکرار بین منابع
    candidates = dedupe_cross_source(
        candidates
    )

    # مرتب‌سازی
    candidates.sort(
        key=lambda x: (
            x.is_urgent,
            x.score,
            x.urgency,
            -age_hours(x.published),
            x.source in TRUSTED_SOURCES,
        ),
        reverse=True,
    )

    # حداکثر یک خبر فوری واقعی
    result = []
    urgent_count = 0

    for item in candidates:

        if item.is_urgent:

            if urgent_count >= URGENT_SLOTS:
                continue

            urgent_count += 1

        result.append(item)

        if len(result) >= MAX_POSTS_PER_RUN:
            break

    return result


# =========================================================
# SAVE STATE
# =========================================================

def save_sent(
    item: NewsItem
):

    link = canonical_url(
        item.url
    )

    title_hash = title_fingerprint(
        item.title
    )

    append_line(
        SENT_LINKS_FILE,
        link
    )

    append_line(
        SENT_TITLES_FILE,
        title_hash
    )


# =========================================================
# RUN ONCE
# =========================================================

def run_once():

    print(
        "=========================================="
    )
    print(
        "JAHANTAB TELEGRAM NEWS BOT"
    )
    print(
        "NEW FINAL VERSION"
    )
    print(
        "=========================================="
    )

    if not BOT_TOKEN:
        print(
            "ERROR: BOT_TOKEN is missing."
        )
        return

    if not CHAT_ID:
        print(
            "ERROR: CHAT_ID is missing."
        )
        return

    sent_links = load_lines(
        SENT_LINKS_FILE
    )

    sent_titles = load_lines(
        SENT_TITLES_FILE
    )

    print(
        f"Already sent links: "
        f"{len(sent_links)}"
    )

    print(
        f"Already sent titles: "
        f"{len(sent_titles)}"
    )

    print(
        "Collecting news..."
    )

    items = collect_items()

    print(
        f"Collected accepted news: "
        f"{len(items)}"
    )

    selected = select_items(
        items,
        sent_links,
        sent_titles
    )

    print(
        f"Selected for publishing: "
        f"{len(selected)}"
    )

    if not selected:

        print(
            "NO NEW NEWS TO PUBLISH"
        )

        return

    for item in selected:

        print(
            "\n------------------------------------------"
        )

        print(
            f"Publishing: {item.title}"
        )

        print(
            f"Source: {item.source}"
        )

        print(
            f"Score: {item.score}"
        )

        print(
            f"Urgent: {item.is_urgent}"
        )

        # اگر عکس RSS نداریم، از صفحه خبر بگیریم
        if not item.image:
            extract_article_data(
                item
            )

        image_path = download_image(
            item.image
        )

        if not image_path:

            print(
                "No valid article image."
            )

            continue

        # افزودن لوگو در صورت وجود
        final_image = add_logo(
            image_path
        )

        sent = send_photo(
            item,
            final_image
        )

        if sent:

            save_sent(
                item
            )

            # برای جلوگیری از ارسال دوباره
            sent_links.add(
                canonical_url(item.url)
            )

            sent_titles.add(
                title_fingerprint(item.title)
            )

            time.sleep(
                POST_DELAY
            )

        # پاک‌سازی تصاویر موقت
        for filename in (
            "_news_image.jpg",
            "_final_news_image.jpg",
        ):

            try:

                if os.path.exists(filename):
                    os.remove(filename)

            except Exception:
                pass

    print(
        "\n=========================================="
    )

    print(
        "RUN FINISHED"
    )

    print(
        "=========================================="
    )


# =========================================================
# MAIN
# IMPORTANT:
# GitHub Actions runs this file every 5 minutes.
# DO NOT use while True here.
# =========================================================

def main():

    run_once()


if __name__ == "__main__":
    main()